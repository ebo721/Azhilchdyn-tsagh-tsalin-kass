...
**How to apply:** Keep header matching exact enough to reject unrelated spreadsheets, but allow the complete header row to appear after a bounded number of preamble rows. Continue applying row and XML size limits. Parse ZIP entries in-process with Node APIs; Vercel serverless functions must not depend on the host having an `unzip` executable.