...
function Payroll() {
  ...
  return <div className="page-enter">
    <PageHeading eyebrow="Сарын тооцоо / payroll" title="Цалингийн задаргаа" detail="Цалинг урьдчилгаа болон сүүл цалин гэж хоёр хуваана. Ээлжийн ажилтны цалинг ажилласан хоногоор тооцно." action={<div className="flex flex-wrap items-center justify-end gap-2"><Button onClick={() => setShowAdvance((value) => !value)} variant={showAdvance ? 'default' : 'outline'} data-testid="button-payroll-advance"><Coins className="size-4" />Урьдчилгаа цалин</Button><div className="flex items-center gap-2 rounded-xl border border-border bg-card px-3"><CalendarDays className="size-4 text-primary" /><input type="month" value={month} onChange={(e) => setMonth(e.target.value)} className="h-10 bg-transparent text-sm outline-none" data-testid="input-payroll-month" /></div></div>} />
