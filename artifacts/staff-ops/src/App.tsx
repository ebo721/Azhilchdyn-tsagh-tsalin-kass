...
function HourBalance() {
  ...
  return <div className="page-enter">
    ...
    {query.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><LoadingBlock className="h-32" /><LoadingBlock className="h-32" /><LoadingBlock className="h-32" /><LoadingBlock className="h-32" /></div> : query.isError ? <ErrorBlock onRetry={() => query.refetch()} /> : <>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3" data-testid="expected-work-days-list">
        {query.data?.map((row) => <div key={row.employeeId} className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3"><div><p className="text-sm font-semibold">{row.employeeName}</p><p className="text-xs text-muted-foreground">{row.role}</p></div><div className="text-right"><p className="font-mono text-lg font-bold text-primary">{row.expectedWorkDays}</p><p className="text-[10px] text-muted-foreground">ажиллах ёстой өдөр</p></div></div>)}
      </div>
  ...
  return <div className="page-enter">
    ...
    {query.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><LoadingBlock className="h-28" /><LoadingBlock className="h-28" /><LoadingBlock className="h-28" /><LoadingBlock className="h-28" /><LoadingBlock className="h-28" /></div> : query.isError ? <ErrorBlock onRetry={() => query.refetch()} /> : <>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard label="Урьдчилгаа цалин" value={money(totalAdvance)} meta="Ээлж: 1–15-ны бүтэн · Оффис: 50%" icon={Coins} />
