...
type EmployeeForm = { name: string; role: string; phone: string; salaryType: 'monthly' | 'hourly'; baseSalary: string; status?: 'active' | 'inactive' };
...
function Employees() {
  ...
  return <div className="page-enter"><PageHeading eyebrow="Хүний нөөц / directory" title="Ажилчид" detail="Багийн бүрэлдэхүүн, цалингийн суурь болон төлвийг нэг дор удирдана." action={<Button onClick={() => setModal({ open: true })} data-testid="button-add-employee"><Plus className="size-4" />Ажилтан нэмэх</Button>} /><section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">Бүх ажилтан <span className="ml-1 font-mono text-xs text-muted-foreground">{query.data?.length ?? 0}</span></p><p className="mt-1 text-xs text-muted-foreground">Идэвхтэй ажилтны мэдээлэл шинэчлэгдэнэ.</p></div><div className="relative w-full sm:w-64"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" placeholder="Нэрээр хайх" data-testid="input-search-employees" /></div></div>{query.isLoading ? <div className="space-y-3 p-5"><LoadingBlock className="h-12" /><LoadingBlock className="h-12" /><LoadingBlock className="h-12" /></div> : query.isError ? <ErrorBlock onRetry={() => query.refetch()} /> : employees.length === 0 ? <EmptyState title="Ажилтан олдсонгүй" detail={search ? 'Хайлтын үгээ өөрчлөөд үзнэ үү.' : 'Эхний ажилтнаа бүртгэж эхлээрэй.'} icon={UsersRound} /> : <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left"><thead className="bg-secondary/50 text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="px-5 py-3 font-bold">Ажилтан</th><th className="px-5 py-3 font-bold">Утас</th><th className="px-5 py-3 font-bold">Цалингийн төрөл</th><th className="px-5 py-3 font-bold">Үндсэн дүн</th><th className="px-5 py-3 font-bold">Төлөв</th><th className="px-5 py-3 text-right font-bold">Үйлдэл</th></tr></thead><tbody className="divide-y divide-border">{employees.map((employee) => <tr className="group transition-colors hover:bg-secondary/35" key={employee.id} data-testid={`row-employee-${employee.id}`}><td className="px-5 py-4"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-accent/30 text-xs font-bold text-foreground">{employee.name.slice(0, 1)}</span><div><p className="text-sm font-semibold">{employee.name}</p><p className="text-xs text-muted-foreground">{employee.role}</p></div></div></td><td className="px-5 py-4 text-sm text-muted-foreground">{employee.phone || '—'}</td><td className="px-5 py-4 text-sm">{employee.salaryType === EmployeeSalaryType.monthly ? 'Сарын' : 'Цагийн'}</td><td className="px-5 py-4 font-mono text-sm">{money(employee.baseSalary)}</td><td className="px-5 py-4"><StatusPill value={employee.status} /></td><td className="px-5 py-4"><div className="flex justify-end gap-1"><button className="grid size-8 place-items-center rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground" onClick={() => setModal({ open: true, employee })} aria-label={`${employee.name} засах`} data-testid={`button-edit-employee-${employee.id}`}><Pencil className="size-4" /></button><button className="grid size-8 place-items-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive" onClick={() => del(employee)} aria-label={`${employee.name} устгах`} data-testid={`button-delete-employee-${employee.id}`}><Trash2 className="size-4" /></button></div></td></tr>)}</tbody></table></div>}</section>{modal.open && <EmployeeModal employee={modal.employee} onClose={() => setModal({ open: false })} />}</div>;
}
...
function Payroll() {
  ...
  return <div className="page-enter"><PageHeading eyebrow="Сарын тооцоо / payroll" title="Цалингийн тойм" detail="Ажилласан өдөр, цаг болон суутгалын дараах цалингийн зургийг харна." action={<div className="flex items-center gap-2 rounded-xl border border-border bg-card px-3"><CalendarDays className="size-4 text-primary" /><input type="month" value={month} onChange={(e) => setMonth(e.target.value)} className="h-10 bg-transparent text-sm outline-none" data-testid="input-payroll-month" /></div>} />{query.isLoading ? <div className="grid gap-4 sm:grid-cols-3"><LoadingBlock className="h-28" /><LoadingBlock className="h-28" /><LoadingBlock className="h-28" /></div> : query.isError ? <ErrorBlock onRetry={() => query.refetch()} /> : <><div className="grid gap-4 sm:grid-cols-3"><StatCard label="Нийт брутто" value={money(query.data?.totalGross)} meta="Суутгалын өмнө" icon={Banknote} /><StatCard label="Нийт суутгал" value={money(query.data?.totalDeductions)} meta="Тооцоолсон суутгал" icon={ArrowDownLeft} tone="orange" /><StatCard label="Олгох цалин" value={money(query.data?.totalNet)} meta="Ажилтнуудад олгох" icon={Coins} tone="gold" /></div><section className="mt-6 overflow-hidden rounded-2xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border px-5 py-4"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.18em] text-primary">Payroll register</p><h2 className="mt-1 text-base font-bold">{month.replace('-', ' оны ')} сарын задаргаа</h2></div><span className="rounded-full bg-secondary px-3 py-1 font-mono text-[10px] font-bold">{query.data?.lines?.length ?? 0} мөр</span></div>{!query.data?.lines?.length ? <EmptyState title="Энэ сард цалин тооцоолоогүй" detail="Ирцийн бүртгэл нэмэгдсэний дараа энд ажилтны мөрүүд гарна." icon={Banknote} /> : <div className="overflow-x-auto"><table className="w-full min-w-[800px] text-left"><thead className="bg-secondary/50 text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="px-5 py-3">Ажилтан</th><th className="px-5 py-3">Ажилласан</th><th className="px-5 py-3 text-right">Брутто</th><th className="px-5 py-3 text-right">Суутгал</th><th className="px-5 py-3 text-right">Олгох</th></tr></thead><tbody className="divide-y divide-border">{query.data.lines.map((line) => <tr key={line.employeeId} className="hover:bg-secondary/35" data-testid={`row-payroll-${line.employeeId}`}><td className="px-5 py-4"><p className="text-sm font-semibold">{line.employeeName}</p><p className="text-xs text-muted-foreground">{line.role}</p></td><td className="px-5 py-4 font-mono text-xs text-muted-foreground">{line.daysWorked} өдөр · {line.hours}ц</td><td className="px-5 py-4 text-right font-mono text-sm">{money(line.gross)}</td><td className="px-5 py-4 text-right font-mono text-sm text-muted-foreground">{money(line.deductions)}</td><td className="px-5 py-4 text-right font-mono text-sm font-bold text-primary">{money(line.net)}</td></tr>)}</tbody></table></div>}</section></>}</div>;
}
