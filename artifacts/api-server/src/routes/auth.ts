...
router.get("/auth/me", (req, res) => {
  ...
  const username = role === "admin" ? "admin" : role === "accountant" ? "saacc" : role === "hr" ? "sahr" : role === "warehouse" ? "satre" : role === "viewer" ? "sasta" : null;
  res.json(role ? { authenticated: true, role, username } : { authenticated: false, role: null, username: null });
});

router.post("/auth/login", (req, res) => {
  const username = typeof req.body?.username === "string" ? req.body.username : "";
  const role: StaffRole | null = username === "admin" ? "admin" : username === "sahr" ? "hr" : username === "saacc" ? "accountant" : username === "satre" ? "warehouse" : username === "sasta" ? "viewer" : null;
  const configuredPassword = role === "admin" ? process.env["ADMIN_PASSWORD"] : role === "hr" ? process.env["HR_MANAGER_PASSWORD"] : role === "accountant" ? process.env["ACCOUNTANT_PASSWORD"] : role === "warehouse" ? process.env["WAREHOUSE_PASSWORD"] : role === "viewer" ? process.env["SASTA_PASSWORD"] : undefined;
